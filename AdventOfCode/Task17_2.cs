...
{
    private static readonly int MAX_CONTINUOUS_STEPS = 10;
    private static readonly int MIN_CONTINUOUS_STEPS = 4;
    ...
    static int ProcessLines(string[] lines)
    {
        ...
        var F = new int[height, width, MAX_NUM_DIRECTIONS, MAX_CONTINUOUS_STEPS + 1];
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                {
                    for (int steps = 1; steps <= MAX_CONTINUOUS_STEPS; steps++)
                    {
    ...
    static int Dijkstra(int height, int width, int[,] a, int[,,,] F)
    {
        ...
        var trace = new (int, int, int, int)[height, width, MAX_NUM_DIRECTIONS, MAX_CONTINUOUS_STEPS + 1];

        while (heap.Count > 0)
        {
            var (currentI, currentJ, currentD, currentSteps) = heap.Dequeue();
            if (currentI == height - 1 && currentJ == width - 1 && currentSteps >= MIN_CONTINUOUS_STEPS)
            {
                // trace
            ...
            for (int dIndex = 0; dIndex < 3; dIndex++)
            {
                ...
                for (int steps = 1; steps <= MAX_CONTINUOUS_STEPS; steps++)
                {
                    ...
                    if (!isWithinBound(nextI, nextJ, height, width) || nextSteps > MAX_CONTINUOUS_STEPS) break;
                    
                    stepsHeat += a[nextI, nextJ];
                    if (MIN_CONTINUOUS_STEPS <= nextSteps && nextSteps <= MAX_CONTINUOUS_STEPS && isWithinBound(nextI, nextJ, height, 
                    width))
                    {
